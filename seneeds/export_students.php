...
if(sizeof($sids)==0){
	...
	if(!$file){
		...
		$worksheet->write(0, 5, 'Yeargroup', $format_hdr_bold);
		$worksheet->write(0, 6, 'Formgroup', $format_hdr_bold);
		$worksheet->write(0, 7, 'DOB', $format_hdr_bold);
		$coloffset=8;
		...
		foreach($sids as $sid){
			...
			$Student=(array)array_merge($Student,$SEN);
			...
			$worksheet->write($rown, 5, $Student['YearGroup']['value'], $format_line_bold);
			$worksheet->write($rown, 6, $Student['RegistrationGroup']['value'], $format_line_bold);
			$worksheet->write($rown, 7, $Student['DOB']['value'], $format_line_bold);
