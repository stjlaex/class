			...
			if(elementObject.className.indexOf("htmleditorarea")!=-1){
				...
				function nice_splice(str, max) {
				  ...
				  tinyMCE.init({
					...
					paste_preprocess : function(pl, o) {                                        
						var maxlenid='maxtextlen'+tinyMCE.activeEditor.id;
						o.content = o.content.slice(0, document.getElementById(maxlenid).value);
						},                                                                      
					paste_postprocess : function(pl, o) {                                                                            
						o.content = o.node.innerHTML;                
						},        
                  	  ...
                  	  function (ed, evt){
						  ...
						  var count=tinyMCE.activeEditor.getContent().replace(/<[^>]+>/g, '').length;
                      	  ...
                      	  if(document.getElementById(lenid).value<=0){
							  ...
							  ed.setContent(tinyMCE.activeEditor.getContent().slice(0, document.getElementById(maxlenid).value));
							  ed.selection.select(ed.getBody(), true);
							  ed.selection.collapse(false);
                      	  }
                  	    });
