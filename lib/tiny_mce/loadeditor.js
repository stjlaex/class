			 ...
			 for(selector in selectors){
				tinyMCE.init({
					...
					paste_preprocess : function(pl, o) {
						...
						if($('#'+maxlenid).length && $('#'+maxlenid).val()>0){
							if($('#'+lenid).val()<=0){
								maxchar=0;
								}
							else{
							...
							o.content = nice_splice(o.content, maxchar);
							var currentlen=maxchar - o.content.length;
							if(currentlen<=0){
								alert("Comment should not be longer than " + $('#'+maxlenid).val() + " characters");
							}
							$('#'+lenid).val(currentlen);
							}
						},
					...
					setup : function (ed){
					  ...
					  ed.onKeyUp.add(function (ed, evt){
						  var lenid='textlen'+tinyMCE.activeEditor.id;
						  var maxlenid='maxtextlen'+tinyMCE.activeEditor.id;
						  if($('#'+maxlenid).length || $('#'+lenid).length){
							  if($('#'+maxlenid).val()>0){
								var char_counter=document.getElementById(maxlenid).value;
								}
							  else{
