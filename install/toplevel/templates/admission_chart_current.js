function admission_chart_current() {
...
function admission_chart(charttype,enrolyear,centername,centerurl) {
	...
	function chart_current(enrolyear) {
		...
		for(var centerno=0; centerno<centernames.length; centerno++){
			...
			var centerlabel = chart
				...
				.on('click', function(){ admission_chart('reenrol',2015,escape(this.textContent),geturl(this.textContent));});
	...
	function chart_reenrol(enrolyear,centername,centerurl) {
		...
		header.append("svg:text")
			...
			.attr("y", 10)
			.text(unescape(centername) + ": Re-enroloment " + enrolyear);
...
function admission_print(centerurl) {
	...
	var url=centerurl+'/class/admin/httpscripts/admissions_print.php?enddate='+date+'&username='+username+'&password='+password;
	xmlHttp=new XMLHttpRequest();
	xmlHttp.open("GET", url, true);
	xmlHttp.onreadystatechange=function(){
		if(xmlHttp.readyState==4 && xmlHttp.status==200){
			var xmlReport=xmlHttp.responseXML;
			var xmlResult=processXML(xmlReport,'admission_tables_long',centerurl+"/templates/");
			openPrintReport(xmlResult,'admission_tables_long','portrait');
			}
		};
	xmlHttp.send();
	}
