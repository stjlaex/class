...
<xsl:template name="averagecell">
  ...
  <td>
	...
	<xsl:if test="$asscount &gt; 0">
	  <xsl:value-of select="count(assessments/assessment[id_db=$id_db]/value[value &gt; 7])" />
	</xsl:if>
  </td>
  <td>
	<xsl:text>&#160; </xsl:text>
	<xsl:if test="$asscount &gt; 0">
	  <xsl:value-of select="count(assessments/assessment[id_db=$id_db]/value[value &gt; 1])" />
	</xsl:if>
  </td>
</xsl:template>
  ...
  <xsl:variable name="asscount" select="count(student/assessments/assessment[id_db=$id_db][subject/value=$bid]/value/value)" />
	  ...
	  <xsl:choose>
		<xsl:when test="$asscount &gt; 0">
		  <td>
			<xsl:value-of select="format-number(count(student/assessments/assessment[id_db=$id_db][subject/value=$bid]/value[value &gt; 7]) div $asscount,'##%')" />
		  </td>
		  <td>
			<xsl:value-of select="format-number(count(student/assessments/assessment[id_db=$id_db][subject/value=$bid]/value[value &gt; 1]) div $asscount,'##%')" />
		  </td>
		</xsl:when>
		<xsl:otherwise>
		  <td>
