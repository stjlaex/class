	...
	for($c=$c_marks-1;$c>-1;$c--) {
		...
		/*********************************************************/
	   	elseif($marktype=='compound'){
			...
			$out='<div class="'.$rep['class'].'" title="'.$outtitle.'" onclick="parent.openModalWindow(\'markbook.php?current=edit_skills.php&cancel=class_view.php&midlist='.$umns[$c]['midlist'].'&pid='.$umns[$c]['component'].'&sid='.$sid.'&bid='.$profilebid.'&colid='.$umns[$c]['id'].'&nextrow='.$rowno.'\',\'\',\'\');" style="cursor:pointer;">'.$outspace.'</div>';
		...
		/*********************************************************/
	   	elseif($marktype=='report'){
			...
			$out='<div onclick="parent.openModalWindow(\'markbook.php?current=edit_single_report.php&cancel=class_view.php&midlist='.$umns[$c]['midlist'].'&pid='.$umns[$c]['component'].'&sid='.$sid.'&bid='.$bid[0].'&colid='.$umns[$c]['id'].'\',\'\',\'\');" style="cursor:pointer;">R';
